#ifndef STK_SHAKERS2_H
#define STK_SHAKERS2_H
...
class Shakers2 : public Instrmnt
{
 public:
...
inline void Shakers2 :: setResonance( BiQuad &filter, StkFloat frequency, StkFloat radius )
{
  filter.a[1] = -2.0 * radius * cos( TWO_PI * frequency / Stk::sampleRate());
  filter.a[2] = radius * radius;
}

inline StkFloat Shakers2 :: tickResonance( BiQuad &filter, StkFloat input )
{
...
inline void Shakers2 :: setEqualization( StkFloat b0, StkFloat b1, StkFloat b2 )
{
  equalizer_.b[0] = b0;
  equalizer_.b[1] = b1;
  equalizer_.b[2] = b2;
...
inline StkFloat Shakers2 :: tickEqualize( StkFloat input )
{
...
inline int Shakers2 :: randomInt( int max ) //  Return random integer between 0 and max-1
{
  return (int) ((float)max * rand() / (RAND_MAX + 1.0) );
}

inline StkFloat Shakers2 :: randomFloat( StkFloat max ) // Return random float between 0.0 and max
{	
  return (StkFloat) (max * rand() / (RAND_MAX + 1.0) );
}

inline StkFloat Shakers2 :: noise( void ) //  Return random StkFloat float between -1.0 and 1.0
{
...
inline void Shakers2 :: waterDrop( void )
{
...
inline StkFloat Shakers2 :: tick( unsigned int )
{
...
inline StkFrames& Shakers2 :: tick( StkFrames& frames, unsigned int channel )
{
